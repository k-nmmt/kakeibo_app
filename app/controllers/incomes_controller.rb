class IncomesController < ApplicationController
  before_action :authenticate_user!, only: [:new, :create, :edit, :update, :destroy]
    ...
    def new
        @income = Income.new
        @incomes = Income.where(user_id:current_user.id).reverse_order.paginate(page:params[:page], per_page: 10)
        @income_month =nil
    ...
    def create
      ...
      def search
        ...
        redirect_to("/incomes/new")
          ...
          @incomes = Income.where(user_id:[current_user.id]).where(income_date: [d.beginning_of_month..d.end_of_month]).where(saving_id:params[:id]).search(params[:income_date]).reverse_order.paginate(page:params[:page], per_page: 10)
